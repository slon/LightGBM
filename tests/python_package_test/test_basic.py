...
class TestBasic(unittest.TestCase):
    ...
    def test_chunked_dataset(self):
        ...
        X_train = [X_train[i * chunk_size:(i + 1) * chunk_size, :] for i in range(X_train.shape[0] // chunk_size)]
        X_test = [X_test[i * chunk_size:(i + 1) * chunk_size, :] for i in range(X_test.shape[0] // chunk_size)]

        print (y_train.shape, [x.shape for x in X_train])
